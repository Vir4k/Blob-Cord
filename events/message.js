...
module.exports = async (client, message) => {
    ...
    if (!timestamps.has(member.id)) {
      ...
      if (now < expirationTime) {
        ...
        return message.channel.send({ embed: { color: 0xcc5353, description: `Calm down dude, please wait **${timeLeft.toFixed(1)}** seconds to try the command again.`} }).then(msg => msg.delete({ timeout: 5000 }))
      }
