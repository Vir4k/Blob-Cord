...
exports.run = async (client, message, args) => {
    ...
    .setColor("RANDOM")
    .setDescription(`<a:loading:732919805254041701> Generating an art!...`)
    let msg = await message.channel
      .send(loadingEmbed)
      .then(m => m.delete({ timeout: 2000 }));

      let random = ["art"]
      let art = random[Math.floor(Math.random() * random.length)]
    ...
    let embed = new Discord.MessageEmbed()
      ...
      .setDescription("**-----------------------------------**")
      .addField("Art provided by", `https://reddit.com/r/${art}`)
...
exports.help = {
  name: "art",
  description: "Checkmate some artists from reddit",
  usage: "art",
  example: "art"
}
